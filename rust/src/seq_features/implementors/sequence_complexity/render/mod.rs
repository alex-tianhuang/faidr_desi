...
-> Result<Graphic, WindowTooLargeError> {
    ...
    let (ln_sum_gamma, _) = special::Gamma::ln_gamma((1 + window_size) as f32);
    let mut counter: AAMap<usize> = AAMap::default();
    let mut ln_gamma_cache: AAMap<f32> = AAMap::default();
    ...
    for (aa, &count) in counter.iter() {
        if count > 0 {
            let (ln_gamma, _) = special::Gamma::ln_gamma((1 + count) as f32);
            ln_gamma_cache[aa] = ln_gamma;
            sum_ln_gamma += ln_gamma;
        };
    }
    let local_complexity = (ln_sum_gamma - sum_ln_gamma) / window_size as f32;

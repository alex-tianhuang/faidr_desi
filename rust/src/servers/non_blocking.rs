...
pub async fn non_blocking_server(
    ...
    loop {
        ...
        let Ok(Request { data }) = from_value(received) else {
            new_task!(sender.send_error("internal error occurred, expected `data` field"));
            continue
        };
        new_task!(forward_request(data, &task_spawner, sender));
    }
}
